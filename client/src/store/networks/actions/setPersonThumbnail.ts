...
const UPLOAD_LIMIT = 1024 * 1024 * 10 // 10 MB Limit
...
export const setPersonThumbnail =
  (networkId: string, personId: string, thumbnail: File | string): AppThunk =>
  async (dispatch, getState) => {
    ...
    try {
      ...
      if (thumbnail instanceof File && thumbnail.size > UPLOAD_LIMIT) {
        window.alert("Image size must be under 10 MB")
        throw new Error("Uploaded thumbnail must be under 10 MB")
      }
